<template>
  <section class="pokeModal">
    <div class="searchBarBox">
      <input class="searchBar" v-model="searchPokemon" type="text" placeholder="Search..." />
    </div>
    <div class="wrapper">
      <div class="wrappedContent">
        <div
          @click="openPokemonDetails(pokemon.url)"
          class="elementArea"
        ...
        >
          <div>
            <div
              :style="{ backgroundColor: typePallete[primaryTypes[index]] }"
              class="pokemonFrame"
            >
              <img
          ...
          <div class="pokemonTypeArea">
            ...
            <div
              v-if="secondaryTypes[index]"
              :style="{ backgroundColor: typePallete[secondaryTypes[index]] }"
              class="typeBox"
            >
...
export default {
  ...
  methods: {
    ...
    try {
      ...
      const dataPromises = this.filteredPokemons.map((pokemon) => axios.get(pokemon.url));
      const dataResponses = await axios.all(dataPromises);

      this.primaryTypes = [];
      this.secondaryTypes = [];
      this.weight = [];

      for (const data of dataResponses) {
        this.primaryTypes.push(data.data.types[0].type.name);
        if (data.data.types[1] !== undefined) {
          this.secondaryTypes.push(data.data.types[1].type.name);
        } else {
          this.secondaryTypes.push('');
        }
        this.weight.push(data.data.weight / 10); // Convertir de hectogramos a kilogramos
      }
  ...
  watch: {
    searchPokemon(newQuery) {
      ...
      this.filteredPokemons = this.info.filter(
        (pokemon) => pokemon.name.includes(query) || pokemon.url.split('/').at(6).includes(query)
      )
      ...
      axios.all(this.endpoints.map((endpoint) => axios.get(endpoint))).then((data) => {
